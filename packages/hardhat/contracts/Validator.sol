...
import {Conquest} from "./Conquest.sol";

contract Validator is FunctionsClient, Ownable {
  using FunctionsRequest for FunctionsRequest.Request;

  Conquest public conquest;
  ...
  event ConquestAddressSet(address newConquestAddress);
  ...
  /**
   * @notice Set the Conquest address
   * @param _conquestAddress New Conquest address
   */
  function setConquestAddress(address _conquestAddress) external onlyOwner {
    conquest = Conquest(payable(_conquestAddress));
    emit ConquestAddressSet(_conquestAddress);
  }
